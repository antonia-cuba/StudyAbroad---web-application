    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            for (int k = -1; k < 2; k++)
            {
                ...
                GxRed += image[i][j].rgbtRed * Gx[k + 1][h + 1];
                GxBlue += image[i][j].rgbtBlue * Gx[k + 1][h + 1];
                GxGreen += image[i][j].rgbtGreen * Gx[k + 1][h + 1];
                GyRed += image[i][j].rgbtRed * Gy[k + 1][h + 1];
                GyBlue += image[i][j].rgbtBlue * Gy[k + 1][h + 1];
                GyGreen += image[i][j].rgbtGreen * Gy[k + 1][h + 1];
                }
            }
