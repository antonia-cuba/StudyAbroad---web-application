    ...
    for (int i = 0; i < height; i++)
    {
        for (int j = 0; j < width; j++)
        {
            ...
            for (int k = -1; k < 2; k++)
            {
                ...
                GxRed += aux[i][j].rgbtRed * Gx[k + 1][h + 1];
                GxBlue += aux[i][j].rgbtBlue * Gx[k + 1][h + 1];
                GxGreen += aux[i][j].rgbtGreen * Gx[k + 1][h + 1];
                GyRed += aux[i][j].rgbtRed * Gy[k + 1][h + 1];
                GyBlue += aux[i][j].rgbtBlue * Gy[k + 1][h + 1];
                GyGreen += aux[i][j].rgbtGreen * Gy[k + 1][h + 1];
                }
            }
            
            int red = sqrt(GxRed * GxRed + GyRed * GyRed);
            int blue = sqrt(GxBlue * GxBlue + GyBlue * GyBlue);
            int green = sqrt(GxGreen * GxGreen + GyGreen * GyGreen);
